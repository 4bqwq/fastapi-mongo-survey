...
async def test_e2e_full_user_path():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        assert publish_resp.json()["data"]["access_url"] == f"/survey/{survey_id}"
        ...
        stats_resp = await client.get(
            f"/api/v1/surveys/{survey_id}/statistics",
            headers=owner_headers,
        )
        ...
        q1_stats = stats["micro_stats"]["q1"]
        assert q1_stats["distribution"]["直接填写数字题"] == 1
        assert q1_stats["distribution"]["继续填写文本题"] == 1

        q2_stats = stats["micro_stats"]["q2"]
        assert q2_stats["text_list"] == ["很好"]
        assert q2_stats["total_answers"] == 1

        q3_stats = stats["micro_stats"]["q3"]
        assert q3_stats["average_value"] == 3.75
        assert q3_stats["valid_answers"] == 2
        assert q3_stats["text_list"] == ["3.5", "4.0"]
