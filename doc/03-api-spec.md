...
- 为了让被共享用户能够立即在前端题库列表中看到该题，系统会在建立共享关系时同步把该题加入被共享用户的题库；后续被共享用户仍可自行移出题库，但不会影响共享权限。
...
- 当某题首次被共享给当前用户时，系统会自动把该题加入当前用户题库，因此该题会立即出现在题库列表中。
...
- 该接口返回问卷基础信息、题目快照与跳转规则，供 C 端填写页和编辑器读取。
- 响应中的 `questions[].versionId` 必须序列化为字符串，不能直接返回 MongoDB `ObjectId` 对象。
...
- 统计维度与题型解释均基于问卷快照。
- 跨问卷统计与题目使用查询由题目相关接口提供，不在本接口内返回。

### 六、当前实现补充

- 当前实现没有提供题目删除与独立的“恢复旧版本”专用接口；若需要回到旧版语义，采用“基于旧版本创建新版本”的方式处理。
